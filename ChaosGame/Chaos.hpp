...
public:
    Chaos(unsigned int, std::mt19937&);
    Chaos(Chaos&) = delete;
    Chaos(Chaos&&) = default;
...
Chaos::Chaos(unsigned int bmpSize, std::mt19937& rGen)
...
auto Chaos::save(std::string path) const -> void {
    this->bmp.saveBitmap(path);
}
auto Chaos::setPoint(POINT& o, POINT v, int i) -> void {
...
*/
auto Chaos::generate(unsigned int runs) -> void { // randomly move to generate Sierpinski Triangle
...
auto Chaos::run(unsigned int depth) -> void { // non randomly move to generate Sierpinski Triangle
    this->recursiveRun(0, depth);
}
auto Chaos::recursiveRun(unsigned int curr, unsigned int depth) -> void {
